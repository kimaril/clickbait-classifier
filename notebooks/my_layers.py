...
class weightedAccCallback(keras.callbacks.Callback):
    ...
    def __init__(self, X1, X2, Y, overlapFeatures_fnc, refutingFeatures_fnc, polarityFeatures_fnc, handFeatures_fnc, 
                 cosFeatures,cosFeatures_two, bleu_fnc, rouge_fnc, cider_fnc_test, X2_two_sentences, overlapFeatures_fnc_two,
                 refutingFeatures_fnc_two, polarityFeatures_fnc_two, handFeatures_fnc_two, bleu_two_sentences, 
                 rouge_two_sentences, cider_two_test, talos_counts_test, talos_tfidfsim_test, talos_svdHeadline_test, 
                 talos_svdBody_test, talos_svdsim_test, talos_w2vHeadline_test, talos_w2vBody_test, talos_w2vsim_test, 
                 talos_sentiHeadline_test, talos_sentiBody_test ):
        self.X1 = X1 
        self.X2 = X2
        self.Y = Y
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        aux = self.model.predict([self.X1, self.X2, self.overlapFeatures_fnc, self.refutingFeatures_fnc, self.polarityFeatures_fnc,\
                                  self.handFeatures_fnc, self.cosFeatures, self.cosFeatures_two, self.bleu_fnc, self.rouge_fnc, \
                                  self.cider_fnc_test, self.X2_two_sentences, self.overlapFeatures_fnc_two, \
                                  self.refutingFeatures_fnc_two, self.polarityFeatures_fnc_two, \
                                  self.handFeatures_fnc_two, self.bleu_two_sentences, self.rouge_two_sentences, self.cider_two_test, \
                                  self.talos_counts_test, self.talos_tfidfsim_test, self.talos_svdHeadline_test, \
                                  self.talos_svdBody_test , self.talos_svdsim_test, self.talos_w2vHeadline_test, \
                                  self.talos_w2vBody_test, self.talos_w2vsim_test ,self.talos_sentiHeadline_test,  \
                                  self.talos_sentiBody_test], batch_size=64)
